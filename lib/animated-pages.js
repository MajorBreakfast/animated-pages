 ...
 * You can implement your own segues via JavaScript and make use of the
 * Web Animations API, jQuery or some other animation library. A segue is
 * basically just an object with a `start()` and a `stop()` method. You can
 * learn more in the documentation of the `createSegue` property or by taking
 * a look at the implementations of the built-in segues.
 *
 * ## Events
 ...
 *
 * You can uses these events to trigger animations, to (re-)load data from a
 ...
 *
 * Info:
 * - All events are fired - even if no segue is played.
 * - If a page change is reverted both pages receive the `'...-will-...'` and
 *   `'...-did-..'` events immediately after each other.
 *
 * @customElement
 * @summary Web component for smooth page transitions
 *
 */
class AnimatedPages extends AnimatedPageSwitcher(PolymerElement) {
  constructor () {
    super()
  ...
  static get properties () {
    return {
       ...
       * - `elements` contains references to the page elements and to the
       *   `<animated-pages>` element. This property is set automatically
       ...
       * relative to it. Furthermore, it set `overflow: hidden` on itself to
       * clip pages that are bigger than the container (can happen during
       * animations).
       *
  ...
  _enqueuePageChange (pageDefinition, createSegue) {
    const promise = super._enqueuePageChange({
      getPageB: () => {
        ...
        // Set page var
        pageB.pageVar = { id, templateName, data }
      ...
      createSegue: ({ pageA, pageB, initialRender }) => {
        const segueInfo = {
          ...
          dataA: pageA && pageA.pageVar.data,
          dataB: pageB && pageB.pageVar.data,
          initialRender
        }
