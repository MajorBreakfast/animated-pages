 ...
 *                 create-animation="_createAnimation">
 ...
 * until the next animation starts, the `<animated-pages>` element will directly
 ...
 *
 * The animation is configured through a function. The `createAnimation`
 ...
 *
 * ```JS
 * import SwapAnimation from 'animated-pages/lib/animations/swap-animation.js'
 * // ...
 * class MyElement extends Element {
 *   // ...
 *   _createAnimation ({ elementA, elementB }) {
 ...
 *
 * The object that is passed into the `createAnimation()` function has
 ...
 * in the description of the `createAnimation` property.
 *
 * ## CSS Animations
 ...
 *
 * ```JS
 * import ClassAnimation from 'animated-pages/lib/animations/class-animation.js'
 * // ...
 * _createAnimation ({ elementA, elementB }) {
...
class AnimatedPages extends AnimatedPageSwitcher(Element) {
  ...
  static get properties () {
    return {
       ...
       *   animation on the initial render can improve the experience in some
       *   scenarios.
       *
       ...
       * ```TS
       * interface Animation {
       *   start: ((done: (result?: AnimationResult) => void) => void) | (() => Promise<AnimationResult?>)
       *   stop?: (canceled, revert) => void
       * }
       *
       ...
       * }
       * ```
       * - The `start()` method is called when the animation should begin
       ...
       *   `done()` callback. Either can be passed an `AnimationResult` object,
       *   which is a plain JavaScript object with an optional `revert`
       *   property. `revert: true` means that the animation ended with pageA
       *   being the shown page. This is useful for animations that are
       *   controlled by user input (i.e. a touch gesture), because the user
       *   might change their mind while they control the animation.
       * - The `stop()` method is called when the animation finishes naturally
       *   or when it is canceled. This function is the last time the animation
       ...
       *   any animation loops (e.g. using `requestAnimationFrame`) it started
       *   in the `start()` method. If the `revert` parameter is `true`, the
       *   animation should restore the state of the elements to the starting
       *   state.
       *
       * ```HTML
       * <animated-pages create-animation="_createAnimation">
       *   ...
       * </animated-pages>
       * ```
       *
      ...
      createAnimation: {
        type: Function,
        observer: '__onCreateAnimationChange'
      },
  ...
  _enqueuePageChange (pageDefinition, createAnimation) {
    const promise = super._enqueuePageChange({
      ...
      createAnimation: ({ pageA, pageB, initialRender }) => {
        const animationInfo = {
        ...
        if (typeof createAnimation === 'function') {
          return createAnimationFn(animationInfo)
        } else if (this.__createAnimationFn) {
          return this.__createAnimationFn(animationInfo)
        } else {
          console.warn(
            `<animated-pages>'s createAnimation property has not been set`)
        }
      },
  ...
  __onCreateAnimationChange (createAnimation) {
    this.__createAnimationFn = this.__functionFromPropertyValue(createAnimation)
  }
