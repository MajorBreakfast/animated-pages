...
const AnimatedPageSwitcher = dedupingMixin(SuperClass => {
  ...
  class AnimatedPageSwitcher extends SuperClass {
    ...
    _enqueuePageChange (callbacks) {
      ...
      const firePageEvent = (page, eventName) => {
        const prefix = this.constructor.is
        const event = new CustomEvent(prefix + '-page-' + eventName)
      ...
      return this.__pageChangeQueue.enqueue({
        start: async () => {
          ...
          if (pageA) { firePageEvent(pageA, 'will-leave') }
          if (pageB) { firePageEvent(pageB, 'will-enter') }
        ...
        stop: (started, canceled, revert) => {
          ...
          if (started) {
            ...
            if (revert) {
              if (hiddenPage) { firePageEvent(hiddenPage, 'will-leave') }
              if (shownPage) { firePageEvent(shownPage, 'will-enter') }
            }

            if (hiddenPage) { firePageEvent(hiddenPage, 'did-leave') }
            if (shownPage) { firePageEvent(shownPage, 'did-enter') }
