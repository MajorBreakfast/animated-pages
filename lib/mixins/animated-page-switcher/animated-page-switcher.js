...
const AnimatedPageSwitcher = dedupingMixin(SuperClass => {
  ...
  class AnimatedPageSwitcher extends SuperClass {
    ...
    _animateToPage (pageB, callbacks) {
      ...
      this.__startOrEnqueuePageSegue(pageSegue)

      return promise
    }

    __startOrEnqueuePageSegue (pageSegue) {
      const doneCallback = () => {
        if (this.__nextPageSegue) { // Start queued segue
          const nextPageSegue = this.__nextPageSegue
          this.__nextPageSegue = undefined
          nextPageSegue.start(doneCallback)
        } else { // Nothing queued => Done animating
          this._setAnimating(false)
        }
      }

      if (this.animating) { // Already animating => Enqueue
        if (this.__nextPageSegue) { this.__nextPageSegue.stop() }
        this.__nextPageSegue = pageSegue
      } else { // Not animating => Start now
        this._setAnimating(true)
        pageSegue.start(doneCallback)
      }
    }
